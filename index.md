...
[![License:
MIT](https://img.shields.io/badge/License-MIT-245f73.svg)](https://hurry060215-tech.github.io/MMV/LICENSE.md)
...
|----|----|
| ![Water Maze Demo](reference/figures/watermaze_demo.png) | ![Minefield Demo](reference/figures/minefield_demo.png) |

Real legacy-file examples (your raw coordinate style):

| SAH (Legacy CSV) | NM (Legacy CSV) |
|----|----|
| ![SAH Real Demo](reference/figures/watermaze_sah4_real.png) | ![NM Real Demo](reference/figures/watermaze_nm5_real.png) |
...
Repository contributors can run `source("scripts/run_examples.R")` for
template conversion, water-maze, minefield, and batch examples from a
source checkout.
...
This script writes: - `man/figures/watermaze_demo.png` -
...
Legacy coordinate-stream CSV is also supported as quoted coordinate
pairs (for example, `"233,135","233,135",...`) or a headerless
two-column numeric CSV. Signed, decimal, and scientific-notation
coordinates are accepted.

## Batch Manifests

A CSV or YAML manifest needs `task` and `input` fields. Relative input
paths are resolved from the manifest file’s directory. Every row returns
an `ok` or `error` status, so one bad file does not stop the remaining
jobs.
...
is deprecated in v0.2.0. It remains available only for compatibility
with existing projects and will be removed in v0.3.0. Pure-R plotting
through
...
is the supported product path. Existing Python users should migrate
their post-processing into an explicit R preprocessing step before the
v0.3.0 removal.

## Windows and Unicode paths

MMV supports spaces and Unicode filenames when R is running with a
UTF-8-capable Windows locale. If tests or conversion report that no CSV
files were found for Chinese filenames, inspect the current process
environment:
...
Some shells incorrectly export the POSIX value `C.UTF-8`, which Windows
R does not recognize. Clear it for the current PowerShell process, then
rerun R:
...
This changes only the current process. MMV does not set global locale
values or silently change user environment variables.

## License

MMV is released under the MIT License. See
[`LICENSE.md`](https://hurry060215-tech.github.io/MMV/LICENSE.md) for
the full license text.

## Contributor release flow

Changes are published through a feature branch and pull request. The
protected `main` branch runs R-CMD-check, coverage, and pkgdown before
merge. A maintainer creates a version tag only after the merged `main`
build is green; the tag workflow creates the source package and GitHub
Release.
...
The workflow regenerates documentation and deploys `main` to the
`gh-pages` branch. GitHub Pages must use that branch in the repository
settings.
